...
void test_ModelARX_brakPobudzenia()
{
	...
	try
	{
		...
		ARXModel instancjaTestowa({ -0.4 }, { 0.6 }, 1, 0);
		...
		for (int i = 0; i < LICZ_ITER; i++)
			faktSygWy[i] = instancjaTestowa.step(sygWe[i]);
...
void test_ModelARX_skokJednostkowy_2()
{
	...
	{
		// Przygotowanie danych:
		ARXModel instancjaTestowa({ -0.4 }, { 0.6 }, 2, 0);
		constexpr size_t LICZ_ITER = 30;
		std::vector<double> sygWe(LICZ_ITER);      // pobudzenie modelu, 
		std::vector<double> spodzSygWy(LICZ_ITER); // spodziewana sekwencja wy
		std::vector<double> faktSygWy(LICZ_ITER);  // faktyczna sekwencja wy
...
void test_ModelARX_skokJednostkowy_3()
{
	...
	{
		// Przygotowanie danych:
		ARXModel instancjaTestowa({ -0.4,0.2 }, { 0.6, 0.3 }, 2, 0);
		constexpr size_t LICZ_ITER = 30;
		std::vector<double> sygWe(LICZ_ITER);      // pobudzenie modelu, 
		std::vector<double> spodzSygWy(LICZ_ITER); // spodziewana sekwencja wy
		std::vector<double> faktSygWy(LICZ_ITER);  // faktyczna sekwencja wy
...
void test_ModelARX_serializationAndDeserialization()
{
	//Sygnatura testu:
	std::cerr << "ModelARX (-0.4 | 0.6 | 1 | 0 ) -> test serializacji i deserializacji: ";
	try
	{
		// Przygotowanie danych:
		ARXModel instancjaTestowa({ -0.4 }, { 0.6 }, 1, 0);
		...
		// Deserializacja:
		ARXModel instancjaTestowa2(ss);

		if (instancjaTestowa == instancjaTestowa2)
			std::cerr << "OK!\n";
		else
		{
			std::cerr << "FAIL!\n";
